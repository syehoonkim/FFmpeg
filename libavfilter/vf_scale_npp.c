...
int ff_cuda_uyvy_to_yuv422p10le_launch(
    AVCUDADeviceContext *hwctx, CUstream stream,
...
static const enum AVPixelFormat deinterleaved_formats[][2] = {
    ...
    {AV_PIX_FMT_UYVY422, AV_PIX_FMT_YUV422P10LE},
};
...
static int config_props(AVFilterLink *outlink)
{
    ...
    AVFilterLink *inlink = IS_SCALE2REF(ctx) ? outlink->src->inputs[1] : outlink->src->inputs[0];
    NPPScaleContext *s = ctx->priv;
    double w_adj = 1.0;
    int ret;
    ...
    if (s->reset_sar)
        w_adj = IS_SCALE2REF(ctx) ? s->var_values[VAR_S2R_MAIN_SAR] : s->var_values[VAR_SAR];
                                 ...
                                 AVFrame *out, AVFrame *in)
{
    ...
    case AV_PIX_FMT_UYVY422:
    {
        ...
        CUstream stream = device_hwctx->stream;

        int ret = ff_cuda_uyvy_to_yuv422p10le_launch(device_hwctx, stream, in->data[0], in->linesize[0], in->width, in->height, (uint16_t *)out->data[0], out->linesize[0], (uint16_t *)out->data[1], out->linesize[1], (uint16_t *)out->data[2], out->linesize[2]);
        if (ret < 0)
        {
            av_log(ctx, AV_LOG_ERROR, "UYVY->UYV422P10LE CUDA deinterleave failed: %d\n", ret);
            return ret;
        }
        break;
    }
                           ...
                           AVFrame *out, AVFrame *in)
{
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(stage->planes_in) && i < FF_ARRAY_ELEMS(in->data) && in->data[i]; i++)
    {
        ...
        const AVPixFmtDescriptor *odesc = av_pix_fmt_desc_get(stage->out_fmt);
        const int out_depth = odesc ? odesc->comp[0].depth : 8;
        if (out_depth <= 8)
        {
            err = nppiResizeSqrPixel_8u_C1R(in->data[i], (NppiSize){iw, ih}, in->linesize[i], (NppiRect){0, 0, iw, ih}, out->data[i], out->linesize[i], (NppiRect){0, 0, ow, oh}, (double)ow / iw, (double)oh / ih, 0.0, 0.0, s->interp_algo);
        }
        else
        {
            err = nppiResizeSqrPixel_8u_C1R((const Npp16u *)in->data[i], (NppiSize){iw, ih}, in->linesize[i], (NppiRect){0, 0, iw, ih}, (Npp16u *)out->data[i], out->linesize[i], (NppiRect){0, 0, ow, oh}, (double)ow / iw, (double)oh / ih, 0.0, 0.0, s->interp_algo);
        }
        if (err != NPP_SUCCESS)
        {
